...
from onemkl import mkl_version, df
...
# Reading the training .csv file using oneAPI DataFrame
df = pd.read_csv("training.csv")
df = df.to_df()

# Replace the values in the imported file using oneAPI DataFrame
...
X = df[l1].to_pandas()
y = df[["prognosis"]].to_pandas().values.ravel()
print(X)
print(y)

# Reading the testing.csv file using oneAPI DataFrame
...
X_test = tr[l1].to_pandas()
y_test = tr[["prognosis"]].to_pandas().values.ravel()
print(X_test)
print(y_test)

def scatterplt():
    # Read the training.csv file using oneAPI DataFrame
    df = pd.read_csv("training.csv")
    df = df.to_df()
    ...
    # Generate the scatter plot using oneAPI DataFrame
    ax.scatter(df['symptom_1'].to_pandas(), df['symptom_2'].to_pandas(), df['prognosis'].to_pandas())

    # Set labels for each axis
    ax.set_xlabel('Symptom 1')
    ax.set_ylabel('Symptom 2')
...
def scatterinp():
    ...
    # Create a new figure and axis for 2D scatter plot
    fig, ax = plt.subplots()

    # Generate the scatter plot using oneAPI DataFrame
    ax.scatter(df['symptom_1'].to_pandas(), df['symptom_2'].to_pandas())
...
w2 = Label(root, justify=LEFT, text="Welcome To Virtual Assistant", fg="Pink", bg="Ivory")
w2.config(font=("Times",30,"bold italic"))
...
# Function to find the nearest hospital based on pincode
def find_nearest_hospital():
    pincode = PincodeEn.get()
    
    # Dictionary of hospital locations with their coordinates
    hospital_locations = {
        'Hospital 1': (latitude1, longitude1),
        'Hospital 2': (latitude2, longitude2),
        'Hospital 3': (latitude3, longitude3),
        # Add more hospitals and their coordinates as needed
    }
    
    # Coordinates of the pincode entered by the user
    user_coordinates = (user_latitude, user_longitude)  # Replace with the actual coordinates based on the pincode
    
    # Initialize variables to track the nearest hospital
    nearest_hospital = None
    nearest_distance = float('inf')
    
    # Iterate over each hospital and calculate the distance from the user's location
    for hospital, location in hospital_locations.items():
        distance = geopy.distance.distance(user_coordinates, location).km
        if distance < nearest_distance:
            nearest_distance = distance
            nearest_hospital = hospital
    
    # Update the label to display the nearest hospital
    nearest_hospital_label.config(text="Nearest Hospital: " + nearest_hospital)

# Label for the pincode
PincodeLb = Label(root, text="Pincode", fg="Red", bg="Ivory")
PincodeLb.config(font=("Times", 15, "bold italic"))
PincodeLb.grid(row=12, column=0, pady=15, sticky=W)

# Taking pincode as input from the user
PincodeEn = Entry(root)
PincodeEn.grid(row=12, column=1)

# Button to find the nearest hospital
find_hospital_btn = Button(root, text="Find Nearest Hospital", command=find_nearest_hospital, bg="Yellow", fg="Purple", width=20)
find_hospital_btn.config(font=("Times", 15, "bold italic"))
find_hospital_btn.grid(row=13, column=1, padx=10, pady=10)

# Label to display the nearest hospital
nearest_hospital_label = Label(root, font=("Times", 15, "bold italic"), text="", height=1, bg="Ivory", width=40)
nearest_hospital_label.grid(row=14, column=0, columnspan=2, padx=10, pady=10)
